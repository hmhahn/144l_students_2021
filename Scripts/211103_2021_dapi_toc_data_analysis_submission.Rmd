...
A1: The control treatment grew the slowest, and was relatively low compared to the other treatments throughout the experiment. The kelp exudate treatment was the next lowest, but had a higher rate of growth towards the beginning of the experiment. For the kelp + nutrients treatment, the rate of growth was higher than both of the other treatments, and the overall cell abundance was much higher. However, the glucose + nutrients treatment had the highest growth rate and cell abundance until day 2, and then the cell abundance decreased slightly unlike the three other treatments. This makes sense considering the glucose treatment had the highest decrease in oxygen and the control had the lowest decrease in oxygen. This correlates to the amount of bacteria using up this oxygen per treatment. 
...
A2: In all of the treatments, the cell biovolume increases then decreases again. The control group had the least variation in biovolume, while it appears the glucose + nutrients treatment and kelp exudate + nutrients treatment had the highest variation, with much higher mean biovolume per cell than the other two treatments during day two. By time point 8, the kelp exudate treatment had the lowest biovolume while the control, glucose, and exudate + nutrients treatment had the next highest in that order.
...
A3: The limitations are that because DAPI only has three timepoints whereas FCM has data from each timepoint, the slope of the curve from exponential growth phase is not as accurate. To get specific growth rate, µ, you must find the slope during the exponential growth phase. It is possible that exponential growth was still occuring slightly past T4, but we would not see that with the DAPI data. Without the specific points from exponential growth phase, we cannot get an accurate µ. 
...
A4: When we account for changes in cell biovolume, the bacterial carbon has a higher peak during T4, but then decreases until T8 (except for the control which just has less of an increase). When biovolume was not accounted for, only the glucose treatment decreased after T4. Accounting for biovolume causes a steeper increase and decrease, causing higher bacterial carbon values during T4 and lowe bacterial carbon values during T8.
...
A5: Yes, the specific growth rates are higher for the treatments that increased cell abundance faster, and are lower for the treatments that had a lower increase in cell abundance. It would make sense that the doubling time is inversely proportional to µ, since the treatments that were growing faster could double their population in a shorter amount of time. The treatments with the highest µ and lowest doubling time also had the largest change in cells since they grew more drastically. This also lines up with the oxygen drawdown data since the groups that had the highest increases of cell abudance used up the most oxygen. 
...
A6: a) The target concentration of carbon to add to each treatment was 10 µM Carbon. 
    b) We did not hit our target in each treatment. 
    c) This implies that we cannot assume the differences in treatment were only influenced by the kind of DOC added, but it could have also been influenced by the starting amount of carbon added.
    d) TOC decreases rapidly to T4, but then decreases more slowly after this time point. This aligns with our other data because during T0-T4, the cells were undergoing rapid growth, meaning they were using up the organic carbon resources. Growth slowed after this time point. 
...
A7: For ∆DOC and Bioavailable DOC, accounting for biovolume does not change the pattern much, except that the kelp exudate + nutrients treatment is slightly higher than just the exudate treatment for the biovolume incorporated data. The values are also slightly higher for the biovolume incorporated data for the exudate, exudate + nutrients, and glucose treatments. The control treatment remained relatively the same. For the BGE data, the control group had relatively the same value after being adjusted for biovolume, but all the others had a large increase. The exudate + nutrient treatment remained the highest, while glucose and exudate treatments were relatively similar for both graphs. The control group had the second highest BGE before being adjusted for biovolume, and after incorporating biovolume, it became the lowest.